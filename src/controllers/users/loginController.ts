...
import { messages } from '../../../config/messages/loginControllerMessages';
...
const loginController: any = async (req: Request, res: Response) => {
  try {
    ...
    return res.status(401).json({ error: messages.unauthorizedAccount });
    ...
    if (!userValidaty)
    return res.status(401).json({ error: messages.invalidUserValidity });
    ...
    res.status(403).json({ error: messages.loginError });
  }
};
