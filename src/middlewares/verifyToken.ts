...
export async function verifyToken(req: Request, res: Response, next: NextFunction): Promise<any> {
  ...
  if (!token)
  return res.status(401).send(messages.jwt.tokenMissing);
  ...
  try {
    ...
    if (!user)
    return res.status(404).send(messages.user.userNotFound);

    if (!user.auth_status)
    return res.status(403).send(messages.account.unauthorizedAccess);

    req.body.user = user;

    next();
  } catch (err) {
    return res.status(401).send(messages.jwt.invalidToken);
  }
}
